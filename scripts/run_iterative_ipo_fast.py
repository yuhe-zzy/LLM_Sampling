...
@torch.no_grad()
def token_level_probabilities(model, tok, prompt, response, max_length, device):
    p_ids = tok(prompt, add_special_tokens=False).input_ids
    y_ids = tok(response, add_special_tokens=False).input_ids
    eos = tok.eos_token_id
    if eos is not None:
        y_ids = y_ids + [eos]
    ...
    if eos_token_id is not None and len(response_token_ids) > 0 and response_token_ids[-1] == eos_token_id and len(tok_lp) > 0:
        ...
        eos_prob = float(tok_prob[-1])

    was_truncated = int((max_length > 0) and (orig_len > max_length))

    return {
        'token_logprobs': tok_lp.tolist(),
        'token_probs': tok_prob.tolist(),
        ...
        'eos_prob': eos_prob,
        'truncated_by_max_length': was_truncated,
    }
...
@torch.no_grad()
def build_augmented_eval_responses(
    model,
    tok,
    prompts_uniq,
    responses_by_prompt,
    ...
    response_sources_by_prompt = [['original'] * len(rs) for rs in responses_by_prompt]
    if augment_num_prompts <= 0 or extra_responses <= 0:
        return responses_by_prompt, u_by_prompt, response_sources_by_prompt, [], []

    rng = random.Random(seed)
    chosen_prompt_indices = sorted(
        rng.sample(range(num_prompts_eval), k=min(int(augment_num_prompts), num_prompts_eval))
    )

    aug_responses = [list(rs) for rs in responses_by_prompt]
    aug_u = [list(us) for us in u_by_prompt]
    augmentation_rows = []
    max_rounds = 4

    model.eval()

    for pid in tqdm(chosen_prompt_indices, desc='augment_eval_generate', ncols=100):
        prompt = prompts_uniq[pid]
        existing_keys = {normalize_text_key(x) for x in aug_responses[pid]}
        candidate_pool = []
        selected = []

        for round_idx in range(max_rounds):
            cand_texts = generate_candidate_responses(
                model=model,
                tok=tok,
                prompt=prompt,
                num_return_sequences=max(1, int(num_generate_candidates)),
            ...
            for txt, ss, aa in zip(uniq_texts, sum_scores, avg_scores):
                ...
                candidate_pool.append({
                    ...
                    'generation_round': int(round_idx),
                })
            ...
            for row in candidate_pool:
                ...
                if key not in dedup:
                    ...
                    old = dedup[key]
                    old_score = old['avg_logprob'] if select_by == 'avg' else old['sum_logprob']
                    new_score = row['avg_logprob'] if select_by == 'avg' else row['sum_logprob']
                    if new_score > old_score:
                        dedup[key] = row

            sorted_pool = sorted(
                dedup.values(),
                key=lambda r: r['avg_logprob'] if select_by == 'avg' else r['sum_logprob'],
                reverse=True,
            )
            ...
            for row in sorted_pool:
                ...
                if len(selected) >= int(extra_responses):
                    break
        ...
        for rank, row in enumerate(selected, start=1):
            aug_responses[pid].append(row['text'])
            aug_u[pid].append(float('nan'))
            response_sources_by_prompt[pid].append('generated')
            augmentation_rows.append({
                'prompt_index': int(pid),
                'prompt_id': int(prompt_ids_raw[pid]),
                'prompt': prompt,
                ...
                'num_original_responses': int(len(responses_by_prompt[pid])),
                'num_added_responses': int(len(selected)),
            })

    return aug_responses, aug_u, response_sources_by_prompt, chosen_prompt_indices, augmentation_rows
    ...
    for pid in range(num_prompts_eval):
        ...
        for j in range(max_k):
            if j < k:
                ...
                row[f'response_source_{j}'] = response_sources_by_prompt[pid][j] if j < len(response_sources_by_prompt[pid]) else 'original'
...
def dump_full_token_diagnostics(
    dump_dir,
    t,
    snapshot_stage,
    prompts_uniq,
    ...
    token_diag_rows = []
    for pid, prompt_text in enumerate(prompts_uniq):
        for j, response_text in enumerate(responses_by_prompt[pid]):
            diag = token_level_probabilities(
                model=model,
                tok=tok,
                prompt=prompt_text,
...
def main():
    ...
    ap.add_argument('--augment_eval_num_prompts', type=int, default=100)
    ap.add_argument('--augment_eval_extra_responses', type=int, default=2)
    ap.add_argument('--augment_eval_num_generate_candidates', type=int, default=20)
    ap.add_argument('--augment_eval_generate_max_new_tokens', type=int, default=256)
    ap.add_argument('--augment_eval_do_sample', type=int, default=1)
    ap.add_argument('--augment_eval_temperature', type=float, default=0.8)
    ap.add_argument('--augment_eval_top_p', type=float, default=0.95)
    ap.add_argument('--augment_eval_select_by', type=str, default='avg', choices=['avg', 'sum'])
    ap.add_argument('--augment_eval_seed', type=int, default=123)
    ...
    if args.augment_eval_num_prompts < 0 or args.augment_eval_extra_responses < 0:
        raise ValueError('augment_eval_num_prompts and augment_eval_extra_responses must be >= 0')
    if args.augment_eval_num_generate_candidates < 1:
        raise ValueError('augment_eval_num_generate_candidates must be >= 1')
    if args.augment_eval_generate_max_new_tokens < 1:
        raise ValueError('augment_eval_generate_max_new_tokens must be >= 1')
    ...
    responses_by_prompt, u_by_prompt, response_sources_by_prompt, augmented_prompt_indices, augmentation_rows = build_augmented_eval_responses(
        model=model,
        tok=tok,
        ...
        augment_num_prompts=args.augment_eval_num_prompts,
        extra_responses=args.augment_eval_extra_responses,
        num_generate_candidates=args.augment_eval_num_generate_candidates,
        max_new_tokens=args.augment_eval_generate_max_new_tokens,
        do_sample=bool(args.augment_eval_do_sample),
        temperature=args.augment_eval_temperature,
        top_p=args.augment_eval_top_p,
        score_batch_size=args.score_batch_size,
        score_max_length=args.max_length,
        select_by=args.augment_eval_select_by,
        seed=args.augment_eval_seed,
        device=device,
    )
    ...
    if args.dump_each_iter == 1 and args.dump_token_diagnostics == 1:
        write_json(
            ...
            {
                'mode': 'all_eval_prompts_all_responses',
                'num_prompts': int(num_prompts_eval),
    ...
    for pid in range(num_prompts_eval):
        for j, resp in enumerate(responses_by_prompt[pid]):
            eval_metadata_rows.append({
                ...
                'prompt': prompts_uniq[pid],
                'response': resp,
                'response_source': response_sources_by_prompt[pid][j] if j < len(response_sources_by_prompt[pid]) else 'original',
    ...
    for t in range(T):
        ...
        if args.dump_each_iter == 1:
            dump_snapshot(
                ...
                prompts_uniq=prompts_uniq,
                responses_by_prompt=responses_by_prompt,
                response_sources_by_prompt=response_sources_by_prompt,
                u_by_prompt=u_by_prompt,
            ...
            if args.dump_token_diagnostics == 1:
                dump_full_token_diagnostics(
                    dump_dir=dump_dir,
                    t=t,
                    snapshot_stage='pre_update',
                    prompts_uniq=prompts_uniq,
