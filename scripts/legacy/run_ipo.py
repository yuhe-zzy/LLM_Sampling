...
def main():
    ...
    if args.eval_support_source == "data":
        ...
        print(
            ...
            f"prompts={len(prompts_uniq)} keep_k={args.eval_response_keep_k}"
        )
    else:
        ...
        print(
            f"[Eval] using generated init responses: prompts={len(prompts_uniq)} "
    ...
    write_json(summary_json, {
        ...
        "artifacts": {
            ...
            "eval_support_note": "dataset_responses" if args.eval_support_source == "data" else "generated_init_only",
        },
    })

    out_csv = os.path.join(
        args.log_dir,
        f"metrics_alpha{args.alpha}_lambda{args.lambda_on}_tau{args.tau}_seed{args.seed}.csv",
    )
    ...
    if args.track_inner_val_loss == 1:
        inner_val_csv = os.path.join(
            args.log_dir,
            f"inner_val_loss_trace_alpha{args.alpha}_lambda{args.lambda_on}_tau{args.tau}_beta{args.beta}_seed{args.seed}.csv",
        )
