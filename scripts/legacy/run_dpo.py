...
def load_data_eval_set(path, num_eval_prompts, seed, keep_k=0):
    rows = read_jsonl(path)
    candidates = []
    for i, r in enumerate(rows):
        prompt = r.get("prompt")
        raw_responses = r.get("responses", [])
        if not isinstance(prompt, str) or not isinstance(raw_responses, list):
            continue
    ...
    if not candidates:
        raise ValueError("No eval rows with dataset responses found in eval_prompts_path.")

    rng = random.Random(seed)
    n = min(int(num_eval_prompts), len(candidates))
    chosen_indices = sorted(rng.sample(range(len(candidates)), k=n))
    chosen = [candidates[i] for i in chosen_indices]

    prompts = [r["prompt"] for r in chosen]
    prompt_ids = [r["prompt_id"] for r in chosen]
    responses_by_prompt = [r["responses"] for r in chosen]
    response_sources_by_prompt = [["dataset_eval"] * len(r["responses"]) for r in chosen]
    return prompts, prompt_ids, responses_by_prompt, response_sources_by_prompt, []
...
@torch.no_grad()
def batch_sum_logprob(model, tok, prompts, responses, max_length, device):
    sequence_sum, _, _ = batch_sum_and_avg_logprob(
        model, tok, prompts, responses, max_length, device
    )
    return sequence_sum
...
def evaluate_fixed_pair_loss(model, ref0, tok, val_ds, batch_size, max_length, device, alpha, beta):
    ...
    ref0.eval()
    for batch in loader:
        lp_c_pi = batch_sum_logprob(model, tok, batch["prompt"], batch["chosen"], max_length, device).to(device)
        lp_r_pi = batch_sum_logprob(model, tok, batch["prompt"], batch["rejected"], max_length, device).to(device)
        lp_c_ref0 = batch_sum_logprob(ref0, tok, batch["prompt"], batch["chosen"], max_length, device).to(device)
        lp_r_ref0 = batch_sum_logprob(ref0, tok, batch["prompt"], batch["rejected"], max_length, device).to(device)
        ...
        s_c, _ = sum_logprob_and_count_from_outputs(out_c.logits, bc["labels"])
        ...
        s_r, _ = sum_logprob_and_count_from_outputs(out_r.logits, br["labels"])

        delta = (s_c - lp_c_ref_t) - (s_r - lp_r_ref_t)
...
def evaluate_fixed_pair_loss_static_ref0(model, ref0, tok, val_ds, batch_size, max_length, device, beta):
    ...
    for batch in loader:
        ...
        delta_static = (s_c - lp_c_ref0) - (s_r - lp_r_ref0)
...
def build_prompt_aware_training_subset(
    ...
    for local_pid, prompt in enumerate(sampled_prompts):
        ...
        # Uniform proposal + one q_t loss weight implements the target pair
        # objective; the constant proposal factor and overall loss scale are
        # removed by the mean normalization below. Sampling from q_t here as
        # well would introduce an unintended second q_t factor.
...
def main():
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "validation-loss trace. The alias --inner_val_steps is kept for backward compatibility."
        ),
    )
    ...
    ap.add_argument("--eval_support_source", type=str, default="data", choices=["data", "generated"])
    ap.add_argument("--eval_response_keep_k", type=int, default=0,
                    help="For --eval_support_source data, keep only the first K dataset responses per prompt. 0 keeps all.")
    ...
    if args.eval_response_keep_k < 0:
        raise ValueError("--eval_response_keep_k must be >= 0")
    ...
    if args.eval_support_source == "data":
        prompts_uniq, prompt_ids_raw, responses_by_prompt, response_sources_by_prompt, generation_rows = load_data_eval_set(
            path=args.eval_prompts_path,
            num_eval_prompts=args.generated_eval_num_prompts,
            seed=args.generated_eval_seed,
            keep_k=args.eval_response_keep_k,
        )
        print(
            f"[Eval] using dataset responses from {args.eval_prompts_path}: "
            f"prompts={len(prompts_uniq)} keep_k={args.eval_response_keep_k or 'all'}"
        )
    else:
        ...
        print(
            f"[Eval] using generated initial-model responses: prompts={len(prompts_uniq)} "
            f"candidates={args.generated_eval_num_candidates} keep_top_k={args.generated_eval_keep_top_k}"
        )
    ...
    for t in range(T):
        ...
        metrics.append({
            ...
            "weight_clip_min": args.w_clip_min,
            "weight_clip_max": args.w_clip_max,
            "effective_on_policy_mass": float((1.0 - args.mix_eps) * args.lambda_on),
            "effective_uniform_exploration_mass": float(
                1.0 - (1.0 - args.mix_eps) * args.lambda_on
            ),
        ...
        for ep in range(args.epochs_per_iter):
            ...
            for batch in pbar:
                with torch.no_grad():
                    lp_c_pi = batch_sum_logprob(model, tok, batch["prompt"], batch["chosen"], args.max_length, device).to(device)
                    lp_r_pi = batch_sum_logprob(model, tok, batch["prompt"], batch["rejected"], args.max_length, device).to(device)
                    lp_c_ref0 = batch_sum_logprob(ref0, tok, batch["prompt"], batch["chosen"], args.max_length, device).to(device)
                    lp_r_ref0 = batch_sum_logprob(ref0, tok, batch["prompt"], batch["rejected"], args.max_length, device).to(device)
    ...
    write_json(summary_json, {
        ...
        "num_prompts_eval": num_prompts_eval,
        "eval_support_source": args.eval_support_source,
        "eval_response_keep_k": args.eval_response_keep_k,
        ...
        "training_rule": {
            ...
            "proposal_distribution": "uniform_over_pairs_with_replacement",
            "target_distribution": "mixed_pair_prob_applied_once_as_loss_weight",
            "lambda_on": args.lambda_on,
            "mix_eps": args.mix_eps,
            "weight_clip_min": args.w_clip_min,
            "weight_clip_max": args.w_clip_max,
        ...
        "evaluation_rule": {
            "source": "dataset_responses" if args.eval_support_source == "data" else "generated_init_only",
            "selection": (
                "use responses stored in eval_prompts_path"
                if args.eval_support_source == "data"
                else "deduplicate then keep top-k by avg_logprob"
            ),
